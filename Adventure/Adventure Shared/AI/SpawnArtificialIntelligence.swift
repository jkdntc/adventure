...
class SpawnArtificialIntelligence: ArtificialIntelligence {
    ...
    override func updateWithTimeSinceLastUpdate(interval timeInterval: TimeInterval) {
        ...
        if shouldGenerateGoblin(timeUntilNextGenerate: &timeUntilNextGenerate) {
            cave.generate()
    ...
    func shouldGenerateGoblin( timeUntilNextGenerate: inout CGFloat) -> Bool {
        ...
        if shouldGenerateGoblinBasedOnCaveDistanceToHero(timeUntilNextGenerate: &timeUntilNextGenerate) {
            return true
        }
    ...
    func shouldGenerateGoblinBasedOnCaveDistanceToHero( timeUntilNextGenerate: inout CGFloat) -> Bool {
        ...
        for hero in scene.heroes {
            ...
            let distance = cavePosition.distanceToPoint(point: heroPosition)
        ...
        if let nearestHeroPosition = nearestHeroPosition {
            ...
            if cave.activeGoblins.count < 5 && distanceScale < 0.24 &&
                cave.timeUntilNextGenerate > 0 && scene.canSee(point: nearestHeroPosition, from: cavePosition) {
